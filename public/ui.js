...
function renderApp() {
  captureViewScrollPosition(state.lastRenderedView);
  captureAddFoodScrollPosition();
  const root = document.getElementById("app");
  const model = buildDashboardModel(state.items);
  const hasBottomNav = state.view !== "notification-settings" && state.view !== "cleanup";
  ...
  root.innerHTML = `
    <div class="mx-auto flex h-[100dvh] min-h-0 w-full max-w-md flex-col overflow-hidden bg-background-light ${hasBottomNav ? "pb-[calc(5.5rem+env(safe-area-inset-bottom))]" : ""} dark:bg-background-dark">
      ${renderCurrentView(model)}
      ${hasBottomNav ? renderBottomNav() : ""}
      ${notificationModal}
      ${detailSheet}
      ${trashDetailSheet}
    </div>
  ...
  restoreViewScrollPosition(state.view);
...
function renderRecentItems(items) {
  ...
  return `
    <main data-view-scroll="dashboard" class="flex-1 overflow-y-auto px-6 pb-[calc(10rem+env(safe-area-inset-bottom))]">
...
function renderBottomNav() {
  ...
  return `
    <nav class="fixed bottom-0 left-0 right-0 border-t border-slate-100 bg-white px-4 pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-2 dark:border-slate-800 dark:bg-slate-900">
...
function getViewScrollArea() {
  return document.querySelector("[data-view-scroll]");
}

function captureViewScrollPosition(viewName) {
  if (!viewName) {
    return;
  }

  const scrollArea = getViewScrollArea();
  if (!scrollArea) {
    return;
  }

  state.viewScrollTops[viewName] = scrollArea.scrollTop;
}

function restoreViewScrollPosition(viewName) {
  if (!viewName) {
    return;
  }
  ...
  scrollArea.scrollTop = state.viewScrollTops[viewName] || 0;
}
...
function renderAllFoodPage(model) {
  return `
    <div class="relative flex min-h-0 flex-1 w-full flex-col overflow-hidden">
      ...
      <main data-view-scroll="all-food" class="custom-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto px-4 pb-[calc(10rem+env(safe-area-inset-bottom))] pt-4">
